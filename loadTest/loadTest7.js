 ...
 for(i=0; i<10; i++) {
  describe('load test 7 ->', function() {
    ...
    it('login with email and password', function() {
      loginPage.setEmail(config.Sa7Email);
    ...
    it('delete user', function() {
      $('=Sa7 User').waitForDisplayed(12000)
      if ($('=Sa7 User').isDisplayed(undefined, true)) {
        $('=Sa7 User').click();
    ...
    it('create class', function() {
      if($(`[data-automation-id=filter-grid-item-link-loadSchool7]`).isDisplayed() === false) {
        homePage.clickAddClass();
        browser.pause(1000);
        homePage.setClassName('loadSchool7');
        homePage.save();
        browser.pause(3000);
        $(`[data-automation-id=filter-grid-item-link-loadSchool7]`).click({
          force: true,
        });
      }
    });
    ...
    it('select other student 1', function() {
      ...
      navbar.selectStudent('Student1, Lock');
    });

    it('remove student 1 from class', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Lock Student1') {
    ...
    it('select other student 2', function() {
      ...
      navbar.selectStudent('Student2, Lock');
    });

    it('remove student 2 from school', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Lock Student2') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select other student 3', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student3, Lock');
    });

    it('remove student 3 from organisation', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Lock Student3') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select move student', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student, Move7');
    });
