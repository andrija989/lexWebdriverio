 ...
 for(i=0; i<10; i++) {
  describe('load test 1 ->', function() {
    ...
    it('login with email and password', function() {
      loginPage.setEmail(config.Sa1Email);
    ...
    it('delete user', function() {
      $('=Sa1 User').waitForDisplayed(12000)
      if ($('=Sa1 User').isDisplayed(undefined, true)) {
        $('=Sa1 User').click();
    ...
    it('create class', function() {
      if($(`[data-automation-id=filter-grid-item-link-loadSchool1]`).isDisplayed() === false) {
        homePage.clickAddClass();
        browser.pause(1000);
        homePage.setClassName('loadSchool1');
        homePage.save();
        browser.pause(3000);
        $(`[data-automation-id=filter-grid-item-link-loadSchool1]`).click({
          force: true,
        });
      }
    });
    ...
    it('select other student 1', function() {
      ...
      navbar.selectStudent('Student1, Fake');
    });

    it('remove student 1 from class', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Fake Student1') {
    ...
    it('select other student 2', function() {
      ...
      navbar.selectStudent('Student2, Fake');
    });

    it('remove student 2 from school', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Fake Student2') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select other student 3', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student3, Fake');
    });

    it('remove student 3 from organisation', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Fake Student3') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select move student', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student, Move1');
    });
