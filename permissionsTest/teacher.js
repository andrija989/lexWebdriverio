...
describe('teacher permisions ->', function() {
  ...
  it('check if authorized', function() {
    allureReporter.addFeature('Permissions test')
    allureReporter.addSeverity('Blocker')
    organisationPage.openUserMenuButton.waitForDisplayed(8000);
    organisationPage.openUserMenu()
  ...
  it('select organisation - no permission', function() {
    allureReporter.addFeature('Permissions test')
    allureReporter.addSeverity('Critical')
    const organisation = navbar.selectOrganisationButton.isDisplayed();
    expect(organisation).to.eql(true);
  });
