 ...
 describe('Role - examiner admin - API automation - login and store token ->', function() {
  ...
  it('Role - examiner admin - go to administration page', function() {
  ...
  var token = ''
  it('Role - examiner admin - get token', function() {
    allureReporter.addFeature('API automation')
  ...
  it('Role - examiner admin - set token', function() {
    var executor = token
    ...
    if (err) throw err;
      color.log('The file with examiner admin token has been saved!', 'success');
    });
  })
 });
