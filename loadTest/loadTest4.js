 ...
 for(i=0; i<10; i++) {
  describe('load test 4 ->', function() {
    ...
    it('login with email and password', function() {
      loginPage.setEmail(config.Sa4Email);
    ...
    it('delete user', function() {
      $('=Sa4 User').waitForDisplayed(12000)
      if ($('=Sa4 User').isDisplayed(undefined, true)) {
        $('=Sa4 User').click();
    ...
    it('create class', function() {
      if($(`[data-automation-id=filter-grid-item-link-loadSchool4]`).isDisplayed() === false) {
        homePage.clickAddClass();
        browser.pause(1000);
        homePage.setClassName('loadSchool4');
        homePage.save();
        browser.pause(3000);
        $(`[data-automation-id=filter-grid-item-link-loadSchool4]`).click({
          force: true,
        });
      }
    });
    ...
    it('select other student 1', function() {
      ...
      navbar.selectStudent('Student1, Mary');
    });

    it('remove student 1 from class', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mary Student1') {
    ...
    it('select other student 2', function() {
      ...
      navbar.selectStudent('Student2, Mary');
    });

    it('remove student 2 from school', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mary Student2') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select other student 3', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student3, Mary');
    });

    it('remove student 3 from organisation', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mary Student3') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select move student', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student, Move4');
    });
