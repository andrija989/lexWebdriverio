 ...
 for(i=0; i<10; i++) {
  describe('load test 5 ->', function() {
    ...
    it('login with email and password', function() {
      loginPage.setEmail(config.Sa5Email);
    ...
    it('delete user', function() {
      $('=Sa5 User').waitForDisplayed(12000)
      if ($('=Sa5 User').isDisplayed(undefined, true)) {
        $('=Sa5 User').click();
    ...
    it('create class', function() {
      if($(`[data-automation-id=filter-grid-item-link-loadSchool5]`).isDisplayed() === false) {
        homePage.clickAddClass();
        browser.pause(1000);
        homePage.setClassName('loadSchool5');
        homePage.save();
        browser.pause(3000);
        $(`[data-automation-id=filter-grid-item-link-loadSchool5]`).click({
          force: true,
        });
      }
    });
    ...
    it('select other student 1', function() {
      ...
      navbar.selectStudent('Student1, Mike');
    });

    it('remove student 1 from class', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mike Student1') {
    ...
    it('select other student 2', function() {
      ...
      navbar.selectStudent('Student2, Mike');
    });

    it('remove student 2 from school', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mike Student2') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select other student 3', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student3, Mike');
    });

    it('remove student 3 from organisation', function() {
      let studentsName = $('h1');
      if (studentsName.getText() == 'Mike Student3') {
        studentPage.removeStudentClick();
        browser.pause(500);
    ...
    it('select move student', function() {
      let student = navbar.selectStudentButton;
      student.waitForEnabled(12000);
      navbar.selectStudent('Student, Move5');
    });
